...
*Đặc tả kỹ thuật yêu cầu AI Agent chạy vòng lặp liên tục, sinh hàng tỷ token dữ liệu pretrain và ghi trực tiếp vào thư mục `E:\DISTILL\`.*

Tài liệu này đóng vai trò là bản hướng dẫn hành động cực đại (Massive Scaling Handoff Prompt) để bạn giao việc trực tiếp cho một AI Agent có khả năng lập trình/ghi file. AI Agent sẽ tự đóng vai trò là bộ máy sản xuất tri thức công nghiệp, chạy vòng lặp liên tục không ngừng nghỉ để sinh hàng triệu dòng dữ liệu sách giáo khoa tiếng Việt chất lượng cao, ghi trực tiếp thành các tệp tin JSONL đánh số tăng dần tại thư mục `E:\DISTILL\` nhằm hướng tới mục tiêu **20 Tỷ đến 40 Tỷ tokens** dữ liệu tiền huấn luyện.

---

## 📂 1. CẤU TRÚC THƯ MỤC & FILE ĐÁNH SỐ TĂNG DẦN (INCREMENTAL STORAGE)

AI Agent phải tạo thư mục `E:\DISTILL\` trên phân vùng hệ thống và phân bổ các tệp tin theo quy tắc đánh số tăng dần để tránh làm quá tải dung lượng của một tệp duy nhất:
...
1. Hãy tạo thư mục `E:\DISTILL\` trên ổ đĩa của bạn (nếu chưa có).
2. Quy định đặt tên file tăng dần để tránh file quá nặng:
   - Toán & Logic: `E:\DISTILL\pretrain_math_0001.jsonl`, `E:\DISTILL\pretrain_math_0002.jsonl`...
   - Giải thuật & Code: `E:\DISTILL\pretrain_cs_0001.jsonl`, `E:\DISTILL\pretrain_cs_0002.jsonl`...
   - Vật lý & Khoa học: `E:\DISTILL\pretrain_physics_0001.jsonl`, `E:\DISTILL\pretrain_physics_0002.jsonl`...
...
5. **CẬP NHẬT MANIFEST LIÊN TỤC**:
   - Bạn phải tạo và duy trì file `E:\DISTILL\distill_manifest.json` ghi lại: tổng số dòng đã sinh, ước tính lượng token đã sinh (trung bình 450 tokens/dòng), thời gian cập nhật và chi tiết sản lượng từng file.
...
7. **ĐỊNH NGHĨA HOÀN THÀNH 100% ĐỂ TRỘN PRETRAIN (100% PRETRAIN-MIX COMPLETION DEFINITION)**:
   - Bạn chỉ được phép công bố 'HOÀN THÀNH 100%' và kết thúc toàn bộ chiến dịch chưng cất khi tổng sản lượng trong `E:\DISTILL\distill_manifest.json` đạt mốc **20 Tỷ tokens** (hoặc chỉ tiêu cụ thể do người dùng cấu hình).
...
Hãy khởi động hệ thống sản xuất, cập nhật file manifest và bắt đầu ghi liên tục các tệp dữ liệu vào thư mục `E:\DISTILL\` ngay bây giờ!
```
